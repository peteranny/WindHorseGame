...
- Every captured monster trails behind the player on the main map (not the mini-map) as a raw, un-decorated icon (no background/border), most-recently-captured closest — like ducklings following their mother, purely for visual flavor, and each mirrored to match the player's own current facing direction. Rather than one slot per grid cell (which reads as jumpy, blocky steps), followers sit at fine, evenly-spaced points resampled directly along the actual pixel path the player walked (not persisted, reset every session), packed close enough together that most of even a large captured roster stays visible on screen at once instead of trailing off past the edge
