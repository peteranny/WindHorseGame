...
Since entering the house is a teleport (`ConversationView`'s `setPosition` above) rather than a walked step, it never goes through `goto`'s own `extendTrail` call - so `Maze/index.tsx` watches `houseState` itself and extends the follower trail by hand the moment it flips to `"occupied"`, using the same `extendTrail` helper a real step would have (needed for correctness later too - once the player leaves, `goto`'s own `extendTrail` call needs the trail's own front cell to already match the goal cell, or a straight-line departure like walking out via the road above wouldn't compute as one). That said, the *closest* follower is resampled starting from the cell *before* the goal (`trail` sliced past its own newly-prepended head) rather than the goal cell itself - resampling from the goal cell would place that follower half a cell in, i.e. straddling into the house's own cell alongside the player, rather than the train visibly waiting outside at the door.
