...
The battle attack line (`Battle/index.tsx`) is the innate attack fixed first, then `gameStore.monsterOrder` (see below) turned into buttons. `Battle/attackGroups.ts`'s `groupByAdjacentFamily` splits that line into maximal runs of consecutive same-family members - each run is one "linked" glow box (`.attackButtonLinked`, a per-button `inset` box-shadow/background driven by `hueForFamily`'s deterministic per-family hue; `.attackGroup`'s wrapper is `display: contents` so grouping never changes the row's own gap/alignment - a cooling-down neighbor is treated as family-less for this computation, since it can't actually be thrown right now and would otherwise wrongly extend the chain). Tapping any member throws the whole run together: the *k*-th member (0-indexed, left-to-right within its own run) contributes `1 + k * step` instead of a flat 1 - so a run of 3 with step 0.5 deals `1 + 1.5 + 2 = 4.5` total, split across however many of the run's members are healers (heal that much, scaled the same way) vs. attackers (damage the wild monster). A real (2+ member) throw also launches each member `GROUP_THROW_STAGGER_MS` (300ms) after the last, so the volley reads as distinguishable throws rather than one blob, and shows a `familyToast` callout ("X 系列攻擊，效果卓越") in the upper half of the battlefield for exactly as long as that whole throw takes to land.

`gameStore.monsterOrder` (front-to-back monster ids) is the single persisted source of both the duckling follower trail (`Maze/index.tsx`) and the battle line above - a battle's own reordering carries over to the map and into the next battle, rather than resetting each time. A new capture prepends its id (`followerTrail.ts`'s `addToOrder`, called from `gameStore.captureMonster`); a save from before this field existed falls back to `orderByMostRecentlyCaptured(captured)` on hydrate. Whenever a *non-innate* group is thrown, each of its members - not the group as one block - alternates between the back and the front of the (captured-monster) line, continuing the same back/front/back/... cycle across every throw in the battle (`Battle/index.tsx`'s `nextPlacement`) - this is deliberate "shuffle": a repeated group throw scatters rather than just gluing itself back together at one end every time. The innate attack is excluded from this cycle entirely and always keeps its fixed first slot. Visually, a thrown button narrows its actual width/padding/border down to 0 (not just a cosmetic transform) before the reorder applies, then grows back the same way at its new spot - so the real flex layout reflows and every other button visibly slides to fill the gap, rather than just fading in place.
