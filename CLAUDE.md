...
The house sprites sit at `z-index: 500` (`Maze/styles.css`) - above the duckling follower trail (whose own z-index tops out at the captured count) so the train always passes behind the house, but below the player pin's `1000` (moot when occupied, since the pin is hidden outright then). They're centered on the goal cell (grown evenly outward in every direction rather than anchored to the cell's bottom) at 120% of the cell's own size.

Since entering the house is a teleport (`ConversationView`'s `setPosition` above) rather than a walked step, it never goes through `goto`'s own `extendTrail` call - so `Maze/index.tsx` watches `houseState` itself and extends the follower trail by hand the moment it flips to `"occupied"`, using the same `extendTrail` helper a real step would have. Without this the trailing duckling followers' path would silently desync from the player's actual cell; with it, the train simply keeps following as always, which in practice means the closest follower ends up right at the doorway rather than inside the house.
