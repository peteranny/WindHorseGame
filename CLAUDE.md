...
Since entering the house is a teleport (`ConversationView`'s `setPosition` above) rather than a walked step, it never goes through `goto`'s own `extendTrail` call - so `Maze/index.tsx` watches `houseState` itself and extends the follower trail by hand the moment it flips to `"occupied"`, using the same `extendTrail` helper a real step would have (needed for correctness later too - once the player leaves, `goto`'s own `extendTrail` call needs the trail's own front cell to already match the goal cell, or a straight-line departure like walking out via the road above wouldn't compute as one). The duckling train follows the player onto the goal cell the same way it would follow onto any other cell - the closest follower ends up half a cell into the house alongside the player, rather than waiting outside at the door.
