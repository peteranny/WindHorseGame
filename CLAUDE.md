...
The only way `position` ever actually becomes the goal cell (normal movement can't reach it - the tile blocks like a wall) is `Battle/index.tsx` calling `gameStore.setPosition` to the goal's coordinates the instant a goal battle is won (same moment `recordGoalWin` fires, before `concludeBattle` swaps the screen back to the post-battle conversation) - that's the "the player sprite enters the house" moment. From there the player can walk back out left or up (both roads on the map bordering the goal tile; right and below are walls) via completely ordinary movement, since `goto`/`findStoppingPoint` only ever check the *destination* cell's passability, never the cell being walked away from. Leaving drops `computeHouseState` back to `"empty"`; re-entering requires talking to the goal tile again (i.e. winning - or skipping via `跳過戰鬥` - another goal battle).
