...
The house sprites sit at `z-index: 500` (`Maze/styles.css`) - above the duckling follower trail (whose own z-index tops out at the captured count) so the train always passes behind the house, but below the player pin's `1000` (moot when occupied, since the pin is hidden outright then).
