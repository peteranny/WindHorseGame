...
Since entering the house is a teleport (`ConversationView`'s `setPosition` above) rather than a walked step, it never goes through `goto`'s own `extendTrail` call - so `Maze/index.tsx` watches `houseState` itself and, the moment it flips to `"occupied"`, *resets* the follower trail to just the house's own cell (rather than extending it). `resamplePath` needs at least 2 cells to place anyone individually, so with only one cell in the trail every follower falls back to the same shared point (`fallbackFollowerPoint`) - the whole duckling train collapses onto that single point, i.e. the entire train (head to tail) visibly enters the house alongside the player, not just the closest few. Leaving is then an entirely ordinary `goto`/`extendTrail` walk starting fresh from that single-cell trail: only as many followers as the walked-so-far path can actually fit get their own resampled point, and the rest stay clamped to the path's last point (still back at the house) - so the tail visibly lags behind, still "in the house," until the player's put enough distance between them for the whole train to have emerged and resumed its usual spread-out trailing.
